...
async function fetchAllNotes() {
    try {
        const snapshot = await db.collection("notes").where("userId", "==", currentUser.uid).get();
        notes = [];
        snapshot.forEach(docRef => notes.push({ id: docRef.id, ...docRef.data() }));
    } catch (e) {
...
function renderLogin() {
    ...
    document.getElementById('google-signin').onclick = () => auth.signInWithPopup(provider).catch(e => alert(e.message));
...
function renderRegister() {
    ...
    document.getElementById('google-signup').onclick = () => auth.signInWithPopup(provider).catch(e => alert(e.message));
...
function renderSettings() {
    ...
    document.getElementById('clear-data').onclick = async () => {
        if (confirm("Are you sure? This will delete ALL your notes permanently.")) {
            toggleSpinner(true);
            const batch = db.batch();
            notes.forEach(note => batch.delete(db.collection("notes").doc(note.id)));
            await batch.commit();
            await fetchAllNotes();
            ...
            notes.forEach(note => {
                ...
                card.innerHTML = `
                    ...
                    <div style="margin-top: auto; padding-top: 1rem; font-size: 0.75rem; color: var(--text-dim); display: flex; justify-content: space-between">
                         ...
                         <span>${note.timestamp ? new Date(note.timestamp.toDate()).toLocaleDateString() : 'Just now'}</span>
                    </div>
                `;
            ...
            document.querySelectorAll('.delete-note').forEach(btn => {
                btn.onclick = async (e) => {
                    ...
                    if (confirm("Delete this record permanently?")) {
                        toggleSpinner(true);
                        await db.collection("notes").doc(e.target.closest('button').dataset.id).delete();
...
function openNoteModal(note = null) {
    ...
    document.getElementById('save-note').onclick = async () => {
        ...
        try {
            if (isEdit) {
                await db.collection("notes").doc(note.id).update({ title, content, category });
...
function startTutorial() {
    modalContainer.classList.remove('hidden');
    modalContainer.innerHTML = `<div class="auth-container"><h2>Tutorial</h2><p>Welcome to Talaga! Use the plus button to add notes.</p><button onclick="document.getElementById('modal-container').classList.add('hidden')" class="btn-primary">Got it</button></div>`;
}
